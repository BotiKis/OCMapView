...
#Background:
You may already encountered the problem when you adding a several hundred annotations the the `MKMapView` it will get laggy and not user friendly.
Many developers believe that iOS can't handle a huge amount of annotations on a `MKMapView` due low memory capacities. So they start to handle the annotation management themselves with pretty dumb filter methods which won't display all annotations and confuse users even more.  

Actually the problem is not memory related. It occurs because *annotationViews* are `UIViews` and these are extremely slow. So if you scroll/zoom a `MKMapView` with many annotations, iOS has to redraw them all at once and that will take time.  

**OCMapView** combines multiple annotations in a specified range and display just ***a single*** `annotationView` for the whole cluster. So you'll never have to many views which make your app slow and laggy. In addition to that forget to handle the annotations yourself! Just add them **all** to your **OCMapView** and it will do everything for you.
...
The `MKAnnotationView` handling stays completely the same so you can use custom Views for your annotations and clusterAnnotations. The `viewForAnnotation` delegate method will return `OCAnnotation` objects when it generated a cluster. It represent clusters and contains all corresponding annotations in it. So you can provide your custom view:
`
...
- Easy to upgrade! You can keep all of your `MKMapView` delegate methods.
- Customize the `MKAnnotationViews` of generated clusters as regular `MKAnnotation`.
